...
The pipeline supports mapping new samples into the latent space of an existing scVI/scANVI/Symphony reference. If it is an scANVI model, this approach allows transferring cell type annotations to new samples. If the scVI/scANVI/Symphony reference was built during a previous run of the pipeline,
...
- **You have a reference scVI model from an arbitrary source (e.g. from a publication) and you want to map new data into the latent space described by the model.** In this case, provide the path via `scvi_model`. Only `scvi` and `scanvi` may be used in `integration_methods`. `scanvi` will only work if the input data contains at least some cell type annotations. Using `scanvi` in addition to `scvi` as an integration method will extend the model so that it can be used for label transfer in future.
- **You have a reference scANVI model from an arbitrary source (e.g. from a publication) and you want to map new data into the latent space described by the model and transfer cell type annotations to the new data.** In this case, provide the path via `scanvi_model`. Only `scanvi` may be used in `integration_methods`.
- **You have a Symphony reference from a previous run of this pipeline and you want to map new data into the latent space described by the reference.** In this case, provide the path via `symphony_model` and use `symphony` in `integration_methods`. If `symphony_model` is omitted, the pipeline will build a new Symphony reference from the current data. The saved reference is an `.h5ad` artifact produced by the pipeline's Python `symphonypy` implementation.
- **You have a reference scVI/scANVI/Symphony model as well as an output AnnData file from a previous run of the pipeline and you want to add more samples to the existing AnnData file.** In this case, provide the corresponding model path via `scvi_model`, `scanvi_model`, or `symphony_model`. If an scANVI model is used, existing cell type annotations will be transferred to the new samples. The existing AnnData file should be provided via the `base_adata` parameter.
