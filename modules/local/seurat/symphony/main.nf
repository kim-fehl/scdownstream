process SEURAT_SYMPHONY {
    ...
    tuple val(meta2), path(reference_model, stageAs: 'reference_model/model.h5ad')
    val(batch_col)

    output:
    tuple val(meta), path("${prefix}.h5ad"), emit: h5ad
    tuple val(meta), path("${prefix}_model.h5ad"), emit: model, optional: true
    ...
    template('symphony.py')

    stub:
    prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch "${prefix}.h5ad"
    touch "${prefix}_model.h5ad"
    touch "X_${prefix}.pkl"
    touch "versions.yml"
    """
}
